...
public class RamAsApproverIssue {
	
	public void ramasapproverissue() throws IOException
	{
		ExtentReport er=new ExtentReport();
	    er.stepReport("RamAsApprover Issue : One Batch with 3 Products");
		Properties prop=new Properties();
		FileInputStream file=new FileInputStream("Properties\\Input.Properties");
		prop.load(file);
		RestAssured.baseURI=prop.getProperty("baseURL");
		String json=utilities.ReadJsonFile.generateStringFromResource("RamAsApproverIssue.json");
    ...
    for(int i=0;i<=2;i++)
    {
 	   if(status[i].equals("Issued"))
 	   {
 		  ...
 		  System.out.println("The RAM as Approver ID"+j+" was issued");
 		 ExtentReport.test.log(Status.PASS, "RamAsApprover Issue - The RamAsApprover ID"+j+" was issued sucessfully");
 		 ExtentReport.child.log(Status.PASS, a.asString());
 		   j++;
 	   }
 	   else
 	   {
 		System.out.println("The RAM as Approver ID"+j+" was not issued");
 		System.out.println("Reason:"+reason[i]);
 		ExtentReport.test.log(Status.PASS, "RamAsApprover Issue - The RamAsApprover ID"+j+" was not issued, Reason:" +reason[i]);
 		 ExtentReport.child.log(Status.PASS, a.asString());
		   j++;
 	   }
    }
	       ...
	       {
	    	   ExtentReport.test.log(Status.FAIL, "RamAsApprover Issue - The Response is : "+a.asString()+" The Status code is : "+a.getStatusCode());
